;;; shell/tools/refactor-move.ss — Move Refactoring
;;;
...
;;;   shell/tools/refactor-integrated.ss (undo infrastructure)
...
;;; Apply the staged move operation.
(define (refactor-move-apply!)
  (if (not *staged-move*)
      ...
      (let* ([info (car *staged-move*)]
            ...
            ;; Backup source file
            (let ([source-content (guard (e [else ""])
                                         (call-with-input-file source-file get-string-all))])
                 (set! backups (cons (cons source-file source-content) backups)))

            ;; Step 1: Remove definition from source
            (printf "  1. Removing definition from ~a...\n" source-file)
            (remove-definition-from-file! source-file sym line-start line-end)

            ;; Backup target file (if exists)
            (when (file-exists? target-file)
                  (let ([target-content (guard (e [else ""])
                                               (call-with-input-file target-file get-string-all))])
                       (set! backups (cons (cons target-file target-content) backups))))

            ;; Step 2: Add definition to target
            (printf "  2. Adding definition to ~a...\n" target-file)
