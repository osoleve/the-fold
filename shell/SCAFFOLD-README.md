...
Creates:
- `shell/string-utils.ss` - Main module file
- `shell/test-string-utils.ss` - Test file
...
Creates:
- `shell/code-stats.ss` - Tool implementation
- `shell/test-code-stats.ss` - Test file

**Important:** After generating a tool, manually add it to `shell/toolkit.ss` registry.
...
Creates:
- `playpen/templates/dungeon-crawler.ss` - Game implementation
...
```bash
scheme --script shell/test-scaffold.ss
```
