;;; shell/diagnostics/README.sexp — Profiling and Diagnostics Modules
;;;
...
Profile an expression:
  (load \"shell/diagnostics/profiler-unified.ss\")
  (profile (expensive-computation))
  (profile-report)

Analyze fuel costs:
  (load \"shell/diagnostics/fuel-analysis.ss\")
  (fuel-costs '(map f xs))
")

 (see-also . (
   "shell/introspect/README.sexp"
   "core/util/debug.ss")))
