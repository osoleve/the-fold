...
(assert-true "has boundary-tool" (assq 'boundary-tool *project-types*))
...
(let ([config (make-config "test-proj" 'boundary-tool "A test" "Alice" #t #t #f)])
     (assert-true "config is alist" (list? config))
     (assert-equal "config name" (assoc-ref config 'name) "test-proj")
     (assert-equal "config type" (assoc-ref config 'type) 'boundary-tool)
...
(let ([config (make-default-config "my-project")])
     ...
     (assert-equal "default config type" (assoc-ref config 'type) 'boundary-tool)
...
(let ([boundary-file (format-main-file "test" "Test tool" 'boundary-tool)])
     (assert-true "boundary file has header"
                  (string-contains? boundary-file "boundary/"))
     (assert-true "boundary file marked as impure"
                  (string-contains? boundary-file "impure")))
