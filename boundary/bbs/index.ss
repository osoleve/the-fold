...
(define (bbs-by-priority priority)
...
(define (bbs-is-blocked? id)
  ...
  (let ([blocker-ids (bbs-blocker-ids id)])
...
(define (bbs-stats)
  ...
  `((total . ,(hashtable-size (bbs-current-items-ht)))
    (open . ,(length (bbs-by-status 'open)))
    (in_progress . ,(length (bbs-by-status 'in_progress)))
    (closed . ,(length (bbs-by-status 'closed)))
    (blocked . ,(length (bbs-blocked)))
    (ready . ,(length (bbs-ready)))
