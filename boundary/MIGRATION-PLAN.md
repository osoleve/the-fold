...
|----|----|----|
| `fs.ss` | 15 files (core/, user/, boundary/) | `io/` |
...
```scheme
;;; boundary/fs.ss — Forwarding stub
;;; The module has moved to boundary/io/fs.ss
...
```bash
mkdir -p boundary/{io,storage,blocks,diagnostics,debug,assistants,media,repl}
```
...
1. **io/fs.ss** - Move file, create stub at `boundary/fs.ss`
2. **storage/store-api.ss** - Move file, create stub at `boundary/store-api.ss`
3. **storage/identity.ss** - Move file, create stub at `boundary/identity.ss`
...
5. **repl/repl.ss** - Move file, create stub at `boundary/repl.ss`
...
After each phase:
```bash
scheme --script boundary/tests/run-tests.ss
```
