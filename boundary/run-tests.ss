;;; boundary/run-tests.ss — Test Runner for Shell Modules
;;;
;;; Runs all boundary/ tests.
;;; Must be invoked from ccverse root directory.
;;;
;;; Usage:
;;;   scheme --script boundary/run-tests.ss   (from ccverse root)
;;;
;;; Dependencies: Requires core/ modules to be loadable

;;; ====
;;; Setup: Ensure we can find core/ and boundary/ modules
;;; ====
...
"))
  ...
  (guard (exn [else
               ...
               (newline)])
         (load (string-append "boundary/" filename))
