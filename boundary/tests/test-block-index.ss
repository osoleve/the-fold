...
(doc 'module 'test-block-index)
(doc 'description "Tests for Block Index - verifies indexing primitives with mutation")
(doc 'layer 'boundary)
(doc 'purity 'partial)
(doc 'note "Run from project root: scheme --script boundary/tests/test-block-index.ss")

(display "Testing boundary/blocks/block-index.ss...\n")

(doc 'section 'empty-index)
(display "  Creating empty index... ")
(let ([idx (make-index)])
     (assert (not (null? idx)))
     (display "✓\n"))

(doc 'section 'empty-stats)
...
(doc 'section 'index-single-block)
...
(doc 'section 'index-multiple-same-tag)
...
(doc 'section 'reference-indexing)
...
(doc 'section 'traverse-references)
