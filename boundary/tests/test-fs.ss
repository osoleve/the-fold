...
(doc 'module 'test-fs)
(doc 'description "Test harness for boundary/io/fs.ss")
(doc 'layer 'boundary)
(doc 'purity 'partial)

(doc 'note "Can't load cas.ss directly — it has relative loads. We just need hash-block.")
(doc 'note "Inline the hash functions we need:")

(doc hash-block 'type (-> Block Bytevector))
...
(doc hash->hex 'type (-> Bytevector String))
...
(doc hex->hash 'type (-> String Bytevector))
(define (hex->hash str)
...
(doc 'note "Use a temporary directory for testing")
(define test-store-path "./test-store")

(doc clean-test-store! 'description "Clean up any previous test run")
...
(doc 'section 'test-create-capability)
...
(doc 'section 'test-store-and-fetch)
...
(doc 'section 'test-content-identity)
...
(doc 'section 'test-block-references)
