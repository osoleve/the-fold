...
(assert-true "test file with path"
             (is-test-file? "boundary/tests/test-string-utils.ss"))
...
(assert-true "finds test file for string-utils"
             (let ([result (find-test-file "boundary/tools/string-utils.ss")])
                  (and result
...
(assert-true "extracts test names from test file"
             (let ([names (extract-test-names "boundary/tests/test-test-finder.ss")])
                  (list? names)))
...
(assert-true "this file references path-basename"
             (test-file-references-symbol? "boundary/tests/test-test-finder.ss" 'path-basename))

(assert-false "this file doesn't reference xyz-nonexistent"
              (test-file-references-symbol? "boundary/tests/test-test-finder.ss" 'xyz-nonexistent-symbol))
