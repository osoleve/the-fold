...
;;;   (load "thimble/commands-example.ss")
...
(register-command!
 ...
 "Check if a string is a valid versioned address (66 hex characters).\n  Usage: (cmd 'validate-hash \"abc123...\")\n         (validate-hash \"abc123...\")"
 (lambda (hash-str)
   (define (hex-char? c)
     (or (char<=? #\0 c #\9)
         (char<=? #\a c #\f)
   ...
   (let ([len (string-length hash-str)]
         [valid? (and (= (string-length hash-str) 66)
                     (string-every hex-char? hash-str))])
     (if valid?
         (begin
           (display "✓ Valid versioned address\n")
           #t)
         (begin
           (display "✗ Invalid hash (must be 66 hex characters)\n")
           #f)))))
