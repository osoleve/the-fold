...
(test "found scripture file"
      #t
      (any (lambda (f) (string-contains? f "scripture/protocol.sexp")) all-sexp-files))
...
(test "forum filter excludes scripture"
      #f
      (any (lambda (f) (string-contains? f "scripture")) forum-files))

(define multi-filter (scan-sexp-files-filtered test-dir '("scripture" "playpen")))
