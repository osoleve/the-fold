...
(load "fabric/stitches/help.ss")  ; For function suggestions
(load "thimble/error-fmt.ss")     ; For base error formatting
  ...
  `(
    ...
    (repl-functions
     ...
     (suggestions "Check if function exists: (help)"
                  "Load help system: (load \"fabric/stitches/help.ss\")"
...
")))
  ...
  (define (get-documentation-links context)
    ...
    (let ([ctx-name (car context)]
         ...
         (append
          (if tutorial
              (list (string-append "📚 Tutorial: (load \"playpen/boardcraft/examples/" tutorial "\")"))
              '())
          (case ctx-name
                [(boardcraft-unbound)
