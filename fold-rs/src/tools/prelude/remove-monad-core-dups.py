#!/usr/bin/env python3
"""Remove duplicate monad-core function definitions from missing.ss."""

import re
from pathlib import Path

# Functions now canonical in monad-core.ss
MONAD_CORE_FUNCTIONS = {
    # State monad
    'state-return', 'state-bind', 'state-get', 'state-put', 'state-modify',
    'run-state', 'eval-state', 'exec-state',
    # Reader monad
    'reader-return', 'reader-bind', 'reader-ask', 'reader-local', 'run-reader',
    # Writer monad
    'writer-return', 'writer-bind', 'writer-tell', 'run-writer'
}
...
def find_function_blocks(content):
    ...
    while i < len(lines):
        ...
        match = re.match(r'^\(([a-z][a-z0-9?!-]*)\s+(\(fn|\(fix|id)', line)
        if match:
            funcname = match.group(1)
            if funcname in MONAD_CORE_FUNCTIONS:
...
def main():
    ...
    for name, occurrences in by_name.items():
        ...
        # Remove ALL occurrences (they're now in monad-core.ss)
    ...
    print(f"\nRemoved {removed_count} monad-core function definitions")
