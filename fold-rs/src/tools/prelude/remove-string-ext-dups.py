#!/usr/bin/env python3
"""Remove duplicate string extension function definitions from missing.ss."""

import re
from pathlib import Path

# Functions now canonical in string-ext.ss
STRING_EXT_FUNCTIONS = {
    'split-when', 'string-empty?', 'string-take', 'string-drop',
    'string-take-right', 'string-drop-right',
    'string-pad-left', 'string-pad-right', 'string-center',
    'string-first', 'string-last', 'string-reverse',
    'string-center-fn'  # Renamed version, also remove
}
...
def find_function_blocks(content):
    ...
    while i < len(lines):
        ...
        if match:
            ...
            if funcname in STRING_EXT_FUNCTIONS:
...
def main():
    ...
    for name, occurrences in by_name.items():
        ...
        # Remove ALL occurrences (they're now in string-ext.ss)
    ...
    print(f"\nRemoved {removed_count} string extension function definitions")
