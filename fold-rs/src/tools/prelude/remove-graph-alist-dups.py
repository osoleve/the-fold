#!/usr/bin/env python3
"""Remove duplicate graph and alist extension function definitions from missing.ss."""

import re
from pathlib import Path

# Functions now canonical in graph-ext.ss and alist-ext.ss
GRAPH_ALIST_FUNCTIONS = {
    # Graph functions
    'graph-add-edge', 'graph-add-undirected-edge', 'graph-neighbors',
    'graph-vertices', 'graph-has-edge?', 'graph-degree',
    'graph-bfs', 'graph-dfs',
    # Alist functions
    'alist-map', 'alist-filter', 'alist-find', 'alist-update', 'alist-merge',
...
def find_function_blocks(content):
    ...
    while i < len(lines):
        ...
        if match:
            ...
            if funcname in GRAPH_ALIST_FUNCTIONS:
...
def main():
    ...
    for name, occurrences in by_name.items():
        ...
        # Remove ALL occurrences (they're now in graph-ext.ss or alist-ext.ss)
    ...
    print(f"\nRemoved {removed_count} graph/alist extension function definitions")
