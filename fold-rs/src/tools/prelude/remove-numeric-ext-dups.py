#!/usr/bin/env python3
"""Remove duplicate numeric extension function definitions from missing.ss."""

import re
from pathlib import Path

# Functions now canonical in numeric-ext.ss
NUMERIC_EXT_FUNCTIONS = {
    'divisors', 'perfect?', 'primes-up-to', 'coprime?', 'totient',
    'is-power-of-2?', 'next-power-of-2', 'log2-int', 'binomial',
    'pow-int', 'digits', 'from-digits', 'digit-sum', 'digit-count',
...
def find_function_blocks(content):
    ...
    while i < len(lines):
        ...
        if match:
            ...
            if funcname in NUMERIC_EXT_FUNCTIONS:
...
def main():
    ...
    for name, occurrences in by_name.items():
        ...
        # Remove ALL occurrences (they're now in numeric-ext.ss)
    ...
    print(f"\nRemoved {removed_count} numeric extension function definitions")
