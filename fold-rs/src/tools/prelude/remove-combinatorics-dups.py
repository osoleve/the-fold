#!/usr/bin/env python3
"""Remove duplicate combinatorics function definitions from missing.ss."""

import re
from pathlib import Path

# Functions now canonical in combinatorics.ss
COMBINATORICS_FUNCTIONS = {
    'rotate-left', 'rotate-right', 'factorial',
    'permutations', 'combinations', 'cartesian-product',
    'power-set', 'list-product'
}
...
def find_function_blocks(content):
    ...
    while i < len(lines):
        ...
        if match:
            ...
            if funcname in COMBINATORICS_FUNCTIONS:
...
def main():
    ...
    # Identify duplicates to remove (keep first, remove rest)
    to_remove = []
    ...
    print(f"\nRemoved {removed_count} duplicate combinatorics function definitions")
