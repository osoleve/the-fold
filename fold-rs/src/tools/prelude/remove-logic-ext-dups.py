#!/usr/bin/env python3
"""Remove duplicate logic extension function definitions from missing.ss."""

import re
from pathlib import Path

# Functions now canonical in logic-ext.ss
LOGIC_EXT_FUNCTIONS = {
    'xor', 'implies', 'iff', 'nand', 'nor'
}
...
def find_function_blocks(content):
    ...
    while i < len(lines):
        ...
        if match:
            ...
            if funcname in LOGIC_EXT_FUNCTIONS:
...
def main():
    ...
    for name, occurrences in by_name.items():
        ...
        # Remove ALL occurrences (they're now in logic-ext.ss)
    ...
    print(f"\nRemoved {removed_count} logic extension function definitions")
