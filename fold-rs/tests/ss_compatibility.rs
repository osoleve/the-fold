...
fn test_top_level_bound_undefined() {
    ...
    assert!(
        ...
        "expected false for undefined symbol, got {:?}",
        result
    );
}
...
fn test_top_level_bound_defined() {
    ...
    assert!(
        ...
        "expected true for defined symbol, got {:?}",
        result
    );
}
...
fn test_unless_false_condition() {
    ...
    assert!(
        matches!(result, Value::Number(42)),
        "expected 42, got {:?}",
        result
    );
}
...
fn test_conditional_load_pattern() {
    ...
    assert!(
        matches!(result, Value::Number(42)),
        "expected 42 when symbol not bound, got {:?}",
        result
    );
    ...
    assert!(
        matches!(result, Value::Nil),
        "expected nil when symbol is bound, got {:?}",
        result
    );
}
...
fn test_short_circuit_unless() {
    ...
    .unwrap();
    assert!(
        matches!(result, Value::Number(99)),
        "expected 99, got {:?}",
        result
    );
}
