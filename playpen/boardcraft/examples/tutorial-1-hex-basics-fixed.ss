;;; playpen/boardcraft/examples/tutorial-1-hex-basics-fixed.ss
;;; BoardCraft Tutorial 1: Hexagonal Board Basics (Fixed)
...
(define cubic-result (axial->cubic axial-coord))
(display "Cubic coordinates: x=")
(display (cubic-coord%-x cubic-result))
(display " y=")
(display (cubic-coord%-y cubic-result))
(display " z=")
(display (cubic-coord%-z cubic-result))
(display "\n")

;; Verify cubic constraint: x + y + z = 0
(display "Verify x+y+z=0: ")
(display (+ (cubic-coord%-x cubic-result)
            (cubic-coord%-y cubic-result)
            (cubic-coord%-z cubic-result)))
(display "\n\n")

;; Convert to offset coordinates (good for storage/display)
(define offset-result (axial->offset axial-coord 'odd-r))
...
;; Place some tiles on the board (conceptually - we'll just show the idea)
(display "You can place tiles on specific hex positions using board-set:\n")
(display "  (board-set board position tile-data)\n\n")

(display "For example:\n")
(display "  (board-set board (axial-coord 0 0) floor-tile)\n")
(display "  (board-set board (axial-coord 1 0) wall-tile)\n")
(display "  (board-set board (axial-coord 2 1) water-tile)\n\n")

;;; ============================================================
;;; Part 6: Ring and Range Calculations
;;; ============================================================
...
(define center-hex (axial-coord 0 0))

;; Find all hexes within 2 steps of center
(define range-2 (hex-range center-hex 2))
(display "All hexes within 2 steps of center: ")
(display (length range-2))
(display " hexes\n")
(display "Positions: ")
(display (take range-2 8))
(display "...\n\n")

;; Find hexes exactly 2 steps away (ring)
(define ring-2 (hex-ring center-hex 2))
(display "Hexes exactly 2 steps away (ring): ")
(display (length ring-2))
(display " hexes\n")
(display "Positions: ")
(display ring-2)
(display "\n\n")
...
(display "✓ You understand rings and ranges\n")
