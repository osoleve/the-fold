...
(let ([spec (call-with-input-file "playpen/satin/examples/scheme-basics.satin" read)])
...
(let ([spec (call-with-input-file "playpen/satin/examples/the-garden.satin" read)])
     (let-values ([(story issues) (satin-compile spec)])
                 (let ([errors (filter satin-error? issues)])
                      (if (null? errors)
                          (begin
