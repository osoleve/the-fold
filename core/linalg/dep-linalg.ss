...
;;; vec-map-typed : Π n A B. (A → B) → Vec n A → Vec n B
;;; Maps a function over a vector, preserving length.
(define (vec-map-typed n A B f v)
  (vec-map f v))

;;; vec-fold-typed : Π n A B. (B → A → B) → B → Vec n A → B
...
;;; matrix-transpose-typed : Π m n A. Matrix m n A → Matrix n m A
;;; Transposes a matrix, swapping dimensions.
(define (matrix-transpose-typed m n A mat)
  (matrix-transpose mat))

;;; matrix-scale-typed : Π m n A. A → Matrix m n A → Matrix m n A
...
;;; vec-ref-safe : Π n A. (i : Nat) → (proof : i < n) → Vec n A → A
...
;;; matrix-ref-safe : Π m n A. (i : Nat) → (j : Nat) → i < m → j < n → Matrix m n A → A
...
;;; vec-nil-typed : Π A. Vec 0 A
;;; Empty vector typed constructor.
(define (vec-nil-typed A)
  (vec))

;;; vec-cons-typed : Π n A. A → Vec n A → Vec (+ 1 n) A
