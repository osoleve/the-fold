...
;;; ============================================================
;;; Differentiability Tracking (Autodiff Integration)
;;; ============================================================

;;; This section provides tools for analyzing annotated ASTs to
;;; identify differentiable code paths for automatic differentiation.
;;;
;;; Key concepts:
;;;   - A type is "differentiable" if it contains (Diff T)
;;;   - An expression is differentiable if its type is differentiable
;;;   - Differentiable paths are expressions that contribute to gradients

;;; type-contains-diff? : Type → Boolean
;;; Check if a type contains (Diff T) anywhere in its structure.
(define (type-contains-diff? t)
  (cond
   [(not (pair? t)) #f]
   [(eq? (car t) 'Diff) #t]
   [else (ormap type-contains-diff? (cdr t))]))

;;; type-contains-grad? : Type → Boolean
;;; Check if a type contains (Grad T) anywhere.
(define (type-contains-grad? t)
  (cond
   [(not (pair? t)) #f]
   [(eq? (car t) 'Grad) #t]
   [else (ormap type-contains-grad? (cdr t))]))

;;; ann-differentiable? : AnnotatedExpr → Boolean
;;; Is this annotated expression differentiable?
(define (ann-differentiable? ann-e)
  (and (ann? ann-e)
       (type-contains-diff? (ann-type ann-e))))

;;; ann-has-gradient? : AnnotatedExpr → Boolean
;;; Does this annotated expression involve gradients?
(define (ann-has-gradient? ann-e)
  (and (ann? ann-e)
       (type-contains-grad? (ann-type ann-e))))

;;; collect-diff-nodes : AnnotatedExpr → (List AnnotatedExpr)
;;; Collect all subexpressions with differentiable types.
(define (collect-diff-nodes ann-e)
  (cond
   [(not (ann? ann-e)) '()]
   [(ann-differentiable? ann-e)
    (cons ann-e (collect-diff-nodes-inner (ann-expr ann-e)))]
   [else (collect-diff-nodes-inner (ann-expr ann-e))]))

(define (collect-diff-nodes-inner e)
  (cond
   [(ann? e) (collect-diff-nodes e)]
   [(not (pair? e)) '()]
   [else (apply append (map collect-diff-nodes-inner e))]))

;;; count-diff-nodes : AnnotatedExpr → Nat
;;; Count differentiable subexpressions.
(define (count-diff-nodes ann-e)
  (length (collect-diff-nodes ann-e)))

;;; diff-input? : AnnotatedExpr → Boolean
;;; Is this a differentiable input (variable with Diff type)?
(define (diff-input? ann-e)
  (and (ann? ann-e)
       (ann-differentiable? ann-e)
       (symbol? (ann-expr ann-e))))

;;; collect-diff-inputs : AnnotatedExpr → (List Symbol)
;;; Collect all variable names that are differentiable inputs.
(define (collect-diff-inputs ann-e)
  (cond
   [(not (ann? ann-e)) '()]
   [(diff-input? ann-e) (list (ann-expr ann-e))]
   [else (collect-diff-inputs-inner (ann-expr ann-e))]))

(define (collect-diff-inputs-inner e)
  (cond
   [(ann? e) (collect-diff-inputs e)]
   [(not (pair? e)) '()]
   [else (apply append (map collect-diff-inputs-inner e))]))

;;; extract-diff-type : Type → Type | #f
;;; If type is (Diff T), return T. Otherwise #f.
(define (extract-diff-type t)
  (if (and (pair? t) (eq? (car t) 'Diff))
      (cadr t)
      #f))

;;; extract-grad-type : Type → Type | #f
;;; If type is (Grad T), return T. Otherwise #f.
(define (extract-grad-type t)
  (if (and (pair? t) (eq? (car t) 'Grad))
      (cadr t)
      #f))

;;; wrap-in-diff : Type → Type
;;; Wrap a type in Diff if not already wrapped.
(define (wrap-in-diff t)
  (if (and (pair? t) (eq? (car t) 'Diff))
      t
      `(Diff ,t)))

;;; unwrap-diff : Type → Type
;;; Remove Diff wrapper if present.
(define (unwrap-diff t)
  (if (and (pair? t) (eq? (car t) 'Diff))
      (cadr t)
      t))
...
;;; analyze-diff : Expr → DiffAnalysis
;;; Analyze an expression for differentiable paths.
;;; Returns an analysis record with:
;;;   - diff-inputs: list of differentiable input variables
;;;   - diff-outputs: whether the output is differentiable
;;;   - diff-ops: count of differentiable operations
(define (analyze-diff expr)
  (let ([result (annotate-expr expr)])
       (if (and (pair? result) (eq? (car result) 'error))
           `(error ,result)
           (let* ([ann-e result]
                  [diff-inputs (collect-diff-inputs ann-e)]
                  [diff-output (ann-differentiable? ann-e)]
                  [diff-count (count-diff-nodes ann-e)])
                 `(diff-analysis
                   (inputs . ,diff-inputs)
                   (output-diff . ,diff-output)
                   (diff-ops . ,diff-count))))))

;;; show-diff-analysis : Expr → Unit
;;; Display differentiability analysis for an expression.
(define (show-diff-analysis expr)
  (let ([analysis (analyze-diff expr)])
       (if (and (pair? analysis) (eq? (car analysis) 'error))
           (begin
            (display "Type error during analysis\n"))
           (begin
            (display "Differentiability Analysis:\n")
            (display "  Inputs:     ")
            (display (cdr (assq 'inputs (cdr analysis))))
            (newline)
            (display "  Output:     ")
            (display (if (cdr (assq 'output-diff (cdr analysis)))
                         "differentiable"
                         "not differentiable"))
            (newline)
            (display "  Diff nodes: ")
            (display (cdr (assq 'diff-ops (cdr analysis))))
            (newline)))))
