...
;;; result-error : Symbol × Symbol × α... → (Result β Error)
(define (result-error phase error-type . details)
  `(error ,phase ,error-type ,@details))

;;; result-suspended : Expr × Env → (Suspended Expr Env)
...
;;; result-context : (Result α Error) → (List α)
(define (result-context r)
  (if (result-ok? r)
      (cddr r)
      (error 'result-context "Not an ok result" r)))

;;; result-phase : (Result α Error) → Symbol
...
;;; result-error-details : (Result α Error) → (List α)
...
;;; adapt-read : String → (Result Expr Error)
...
;;; adapt-parse : String × String... → (Result Expr Error)
...
;;; adapt-normalize : Expr → (Result Expr Error)
...
;;; adapt-expand : Expr → (Result Expr Error)
...
;;; adapt-infer : Expr → (Result Type Error)
...
;;; adapt-eval : Expr × Nat → (Result Value Error)
...
;;; compile : String × α... → (Result Value Error)
...
;;; compile-pipeline : String × Symbol × Nat → (Result Value Error)
...
;;; compile-exprs : (List Expr) → (Result Value Error)
...
;;; compile-expr : Expr → (Result Value Error)
