;;; fabric/stitches/reverse-diff.ss — Reverse Mode Automatic Differentiation
;;;
...
;;; reverse-tape-push! : MutableTape × TapeEntry → Void
...
;;; make-traced-var : Number × MutableTape → Traced
...
;;; make-traced-const : Number × MutableTape → Traced
...
;;; traced-add : Traced|Number × Traced|Number → Traced
;;; Addition with gradient tracking. ∂(a+b)/∂a = 1, ∂(a+b)/∂b = 1
...
;;; traced-sub : Traced|Number × Traced|Number → Traced
...
;;; traced-mul : Traced|Number × Traced|Number → Traced
...
;;; traced-div : Traced|Number × Traced|Number → Traced
...
;;; traced-pow : Traced|Number × Number → Traced
;;; Power. ∂(a^n)/∂a = n*a^(n-1)
...
;;; backward : MutableTape × Nat × Number → Hashtable
...
;;; gradient : ((Traced ...) → Traced) × (List Number) → (List Number)
...
;;; gradient-at : (Number × ... → Number) × Number × ... → (List Number)
;;; Variadic version of gradient.
(define (gradient-at f . args)
  (gradient (lambda (xs) (apply f xs)) args))

;;; value-and-gradient : ((Traced ...) → Traced) × (List Number) → (Values Number (List Number))
...
;;; reverse-diff : (Traced → Traced) × Number → Number
...
;;; numerical-gradient : ((Number ...) → Number) × (List Number) × Number → (List Number)
...
;;; check-gradient : (List Number → Number) × (List Number) × Number → Boolean
