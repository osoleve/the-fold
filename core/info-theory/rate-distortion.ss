...
;;; squared-error : Number × Number → Number
...
;;; mae : (List Number) × (List Number) → Number
...
;;; rmse : (List Number) × (List Number) → Number
;;; Root mean squared error: sqrt(MSE)
(define (rmse xs ys)
  (sqrt (mse xs ys)))

;;; normalized-mse : (List Number) × (List Number) → Number
...
;;; snr : (List Number) × (List Number) → Number
...
;;; snr-db : (List Number) × (List Number) → Number
...
;;; psnr : (List Number) × (List Number) × Number → Number
...
;;; gaussian-rate-distortion : Number × Number → Number
...
;;; gaussian-distortion-rate : Number × Number → Number
...
;;; binary-rate-distortion : Number → Number
...
;;; binary-distortion-rate : Number → Number
...
;;; binary-entropy-inverse : Number → Number
...
;;; uniform-quantize : Number × Number × Int → Number
...
;;; uniform-quantize-list : (List Number) × Number × Int → (List Number)
...
;;; uniform-quantization-distortion : Number × Int → Number
...
;;; lloyd-max-quantizer : (List Number) × Int × Int → (List Number)
...
;;; uniform-init-levels : Number × Number × Int → (List Number)
...
;;; lloyd-max-iterate : (List Number) × (List Number) × Int → (List Number)
...
;;; compute-boundaries : (List Number) → (List Number)
...
;;; compute-centroids : (List Number) × (List Number) → (List Number)
...
;;; partition-by-boundaries : (List Number) × (List Number) → (List (List Number))
...
;;; in-region? : Number × Int × (List Number) → Boolean
...
;;; region-centroid : (List Number) → Number
...
;;; levels-converged? : (List Number) × (List Number) × Number → Boolean
...
;;; lloyd-max-quantize : Number × (List Number) → Number
...
;;; vq-codebook-distance : (List Number) × (List Number) → Number
;;; Euclidean distance between two vectors.
(define (vq-codebook-distance v1 v2)
  (sqrt (fold-left + 0 (map squared-error v1 v2))))

;;; vq-find-nearest : (List Number) × (List (List Number)) → Int
...
;;; vq-quantize : (List Number) × (List (List Number)) → (List Number)
...
;;; operational-rate-distortion : (List Number) × (List Number) → (Number × Number)
...
;;; rd-gap : Number × Number × Number × Number → Number
...
;;; quantizer-entropy : (List Number) → Number
...
;;; count-values : (List a) → (List (a . Int))
...
;;; entropy-coded-rate : (List Number) → Number
...
;;; dither-quantize : Number × Number × Int × Number → Number
...
;;; high-rate-bits-per-sample : Number × Number → Number
...
;;; high-rate-distortion : Number × Number → Number
...
;;; rd-summary : (List Number) × (List Number) × Number → String
