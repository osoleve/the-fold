;;; core/sim/README.sexp --- Simulation Stream Abstraction Module
...
(module sim
  ...
  (usage-examples
    ((simulate-projectile
      "Create projectile simulation under gravity"
    ...
    (unfold-fibonacci
      ...
      (let ([step (lambda (pair)
                    (cons (car pair)
                          (cons (cdr pair) (+ (car pair) (cdr pair)))))])
        (stream->list 10 (sim-unfold-infinite step (cons 0 1)))))

    (running-sum
      "Compute running sum of simulation data"
      (sim-scan + 0 (sim-times simulation-stream)))

    (energy-tracking
      "Track energy throughout simulation"
      (sim-total-energy 9.8 0 physics-stream))))

  (dependencies
    ("core/base/prelude.ss" "Base utilities")
    ("lattice/fp/data/stream.ss" "Lazy stream primitives")
    ("lattice/linalg/vec2.ss" "2D vector math"))

  (design-notes
    (pure-simulation
      "Unlike user/physics/world.ss which uses mutable state,
       simulation-stream.ss provides purely functional simulation.
       State is never mutated; each step produces a new state.")

    (infinite-streams
      "Simulations are modeled as potentially infinite streams.
       Use stream-take, sim-until, or fuel parameters to bound
       computation when needed.")

    (codata-pattern
      "This module follows the codata pattern from stream.ss.
       Simulations are defined by their observations (step function)
       rather than their construction (finite data).")

    (integration-methods
      "Three integration methods are provided:
