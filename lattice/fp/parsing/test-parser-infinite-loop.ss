...
(test-group infinite-loop-tests
            ...
            (define-test many-no-consume-test
              (let* ([p (parser-pure 42)]
                     [result (parser-parse (parser-many p) "hello")])
            ...
            (define-test some-no-consume-test
              (let* ([p (parser-pure 42)]
                     [result (parser-parse (parser-some p) "hello")])
            ...
            (define-test many-optional-test
              (let* ([p (parser-optional (parser-char (integer->char 97)) #\x)]  ; optional 'a'
                     [result (parser-parse (parser-many p) "bbb")])
            ...
            (define-test many-normal-test
              (let* ([p parser-digit]
                     [result (parser-parse (parser-many p) "123abc")])
            ...
            (define-test some-normal-test
              (let* ([p parser-digit]
                     [result (parser-parse (parser-some p) "456xyz")])
                    ;; Should consume all digits
                    (assert-true (right? result))
                    (assert-equal 3 (length (from-right result)))))
            ...
            (define-test many-empty-input-test
              (let* ([p parser-digit]
                     [result (parser-parse (parser-many p) "")])
                    (assert-true (right? result))
                    (assert-equal '() (from-right result))))
            ...
            (define-test some-empty-input-test
              (let* ([p parser-digit]
                     [result (parser-parse (parser-some p) "")])
                    (assert-true (left? result))))
            
            ;; Test many with look-ahead (doesn't consume)
            (define-test many-lookahead-test
              (let* ([p (parser-look-ahead (parser-char (integer->char 97)))]  ; lookahead 'a'
                     [result (parser-parse (parser-many p) "aaa")])
            ...
            (define-test nested-many-test
              (let* ([inner (parser-many (parser-pure 1))]
                     [outer (parser-many inner)]
                     [result (parser-parse outer "test")])
...
(test-group regression-tests
            ...
            (define-test spaces-test
              (let ([result (parser-parse parser-spaces "   abc")])
                   (assert-true (right? result))
                   (assert-equal "   " (from-right result))))
            
            ;; Test spaces1 combinator (uses some)
            (define-test spaces1-test
              (let ([result (parser-parse parser-spaces1 "  xyz")])
                   (assert-true (right? result))
                   (assert-equal "  " (from-right result))))
            
            ;; Test identifier (uses many)
            (define-test identifier-test
              (let ([result (parser-parse parser-identifier "foo123")])
                   (assert-true (right? result))
                   (assert-equal "foo123" (from-right result))))
            
            ;; Test natural number (uses some)
            (define-test natural-test
              (let ([result (parser-parse parser-natural "42abc")])
                   (assert-true (right? result))
                   (assert-equal 42 (from-right result))))
            
            ;; Test sep-by (uses many internally)
            (define-test sep-by-test
              (let* ([p (parser-sep-by parser-digit (parser-char (integer->char 44)))]  ; comma
                     [result (parser-parse p "1,2,3,4")])
                    (assert-true (right? result))
