...
(define (make-diagonal-constraint q1 q2 diff)
  (lambda (cs)
          ;; After labeling, check that |q1 - q2| ≠ diff
          ;; During propagation, we constrain using auxiliary vars
          (let* ([q1-val (cstore-get-value cs q1)]
                 [q2-val (cstore-get-value cs q2)])
                (if (and q1-val q2-val)
                    ;; Both ground - check directly
                    (if (= (abs (- q1-val q2-val)) diff)
                        (clp-fail cs)
                        (clp-succeed cs))
                    ;; Not both ground - add constraint for later
                    ;; Use =/=fd on the difference possibilities
                    (let* ([d-plus (make-lvar 'd+)]
                           [d-minus (make-lvar 'd-)]
                           ;; d+ = q2 + diff, d- = q2 - diff
                           ;; We want q1 ≠ d+ AND q1 ≠ d-
                           ;; Create constraint using the FD system
                           [cs1 ((=/=fd-diagonal q1 q2 diff) cs)])
                          (if cs1
                              (clp-succeed cs1)
                              (clp-fail cs))))))
