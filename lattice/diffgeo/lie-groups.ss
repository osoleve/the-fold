;;; lattice/diffgeo/lie-groups.ss — Lie Groups and Algebras
;;; @module lie-groups
;;; @requires prelude matrix vec matrix-solvers
;;;
;;; Rotation and transformation groups with their Lie algebras.
;;;
;;; A Lie group G is a smooth manifold that is also a group. The tangent space
;;; at the identity element forms the Lie algebra 𝔤, which captures the
;;; "infinitesimal" structure of the group.
;;;
;;; Key insight: The exponential map exp: 𝔤 → G converts infinitesimal motions
;;; (like angular velocity) into finite transformations (like rotations).
;;;
;;; Implemented groups:
;;;   SO(2) - 2D rotations (circle group)
;;;   SO(3) - 3D rotations (rotation matrices)
...
(define *lie-epsilon* 1e-10)  ; Tolerance for numerical comparisons
...
;;; ============================================================================
;;;
;;; SO(2) is the group of 2×2 orthogonal matrices with determinant 1.
;;; Each element is a rotation by angle θ:
;;;
;;;   R(θ) = [cos θ  -sin θ]
;;;          [sin θ   cos θ]
;;;
;;; The Lie algebra so(2) consists of 2×2 skew-symmetric matrices:
;;;
;;;   ω̂ = [ 0  -ω]     (generated by single element ω ∈ ℝ)
;;;       [ ω   0]
;;;
;;; Exponential map: exp(θ · ĝ) = R(θ) where ĝ is the generator.

;;; --- SO(2) Group Elements ---

;;; so2? : Any → Boolean
;;; Check if x is an SO(2) element (2×2 rotation matrix).
(define (so2? x)
  (and (pair? x)
       (eq? (car x) 'so2)
       (= (length x) 2)
